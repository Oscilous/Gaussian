...
def generate_gaussian_mask(shape, std_dev):
    y, x = np.indices(shape)
    center_x, center_y = 830, 620
    return np.exp(-((x - center_x) ** 2 + (y - center_y) ** 2) / (2 * std_dev ** 2))
...
try:
    ...
    update_mask()
    gaussian_mask = generate_gaussian_mask(pellet_center_mask.shape, std_dev)
    gaussian_pellet_mask = np.multiply(pellet_center_mask, gaussian_mask)
    subtracted_image = original_image - gaussian_pellet_mask
    subtracted_image[subtracted_image < 0] = 0  # Ensure there are no negative values

    cv2.imwrite("gaussian.jpg", gaussian_pellet_mask )
